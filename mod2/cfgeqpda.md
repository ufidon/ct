...
- run `aab` on the PDA.
  ...
  - run `aab` on the PDA following the same sequence of productions
    - keep the STACK contents the same as the string of nonterminals in the working string
...
- All branching, deterministic, or nondeterministic, occurs at RoH states, none at POP states, and every edge has only one label (no multiple labels)
  -  ❺ convert all branching at POP states into branching at RoH states
- `❺ case 1`: 
```mermaid
flowchart LR
  un1(("⋯"))
...
---
-  PDA conversion form is a collection of `path segments`, each of the fol­lowing form
...
- in row 2, `PUSH a$` is a short notation of `PUSH $` then `PUSH a`
...
- Every word accepted by the PDA corresponds to some `path` from START to ACCEPT
  ...
  - e.x. `aaaabb` can be accepted by the path `row(1,2,3,3,3,4,5,6,5,7)`
