...
flowchart LR
  ...
  po3-->|b|r2
```
- (p11) a deterministic PDA for PALINDROMEX
- (p12) a nondeterministic PDA for PALINDROMEX without `REJECT` states
  - run `abbXbba`
...
- (p13) so we can reuse the previous PDA by changing X into "a,b", 
...
flowchart LR
  ...
  po3-->|b|r2
```
- (p14) find a path for `aba`


🍎 Example 5
---
A PDA accepts the language 
- (p15) EVENPALINDROME = {s reverse(s), where s is in $\mathbf{(a + b)^*}$}
...
🍎 Example 6
---
A PDA accepts the language generated by the CFG (p18)
- S → S + S | S*S | 4
  - `+,*,4` are the terminals
- (p19-20) trace the acceptance of the string `4+4*4`
```mermaid
flowchart LR
  s(["START"])
  ac(["ACCEPT"])
