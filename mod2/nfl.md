...
NONNULLPALINDROME has words shorter than 6 letters with self-embedding nonterminal such as `aabaa` (p8):
...
- S₁→S₂→S₃→⋯→Sₙ can be short-noted as `S₁⊨Sₙ` or ${S_1 \stackrel{*}{⇒} S_2}$
...
☯ Theorem 1: The pumping lemma for CFLs
---
...
Proof 🅰:
- if length(w)>2ᵖ, then there are always `self-embedded nontenninals` in `any derivation tree for w`.
- given such as derivation tree and one self-embedded nontenninals P whose first production is P→QR (p10①)
...
Proof 🅱:
- If P is a self-embedded nontenninal in the derivation of w, then there are substrings u,v,x≠ε,y,z, and v,y are not both ε, such that
  - S ⊨ uPz
  - P ⊨ vPy
  - P ⊨ x
...
☯ Theorem 4: The pumping lemma for CFLs with length
---
